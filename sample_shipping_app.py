...
st.markdown("### 填写寄送信息")
st.info(
    "固定写入内容：客户code = KH00832，客户名称 = 第三方样品，子订单类型 = 样品。"
    " 支付日期会继续保留模板中的 TODAY() 公式。"
)
...
if submitted:
    ...
    if not active_manual_groups and not batch_rows:
        ...
        try:
            ...
            output_file = build_output_file(template_path=TEMPLATE_PATH, address_groups=parsed_address_groups)
